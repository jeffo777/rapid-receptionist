...
| **FloatingWidget** | Persistent bottom-right AI avatar widget — appears after scrolling past hero, expands to show Chat/Call CTAs |
...
- **`public/robots.txt`** — allows all crawlers, points to sitemap, blocks `/_astro/` and `/api/`
- **Sitemap** — auto-generated by `@astrojs/sitemap` at build time; `astro.config.mjs` sets `site: 'https://rapidreceptionist.co.uk'`
- **Schema markup** — JSON-LD (FAQPage, Service, BreadcrumbList) auto-generated per page by layout files
- **`public/llms.txt`** and **`public/llms-full.txt`** — AI/LLM-readable site documentation
...
│   └── llms-full.txt               — Extended AI/LLM site details
├── pages/
│   ├── index.astro
│   ├── pricing.astro
│   ├── about.astro
